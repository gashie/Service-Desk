const { DetectDevice, DetectIp } = require("../helper/devicefuncs");
...
const logMiddleware = async (req, res, next) => {
  ...
  res.on("finish", () => {
    ...
    const logData = {
      service_name: process.env.ServiceName || "Service_Desk",
...
const errorHandlerMiddleware = (err, req, res, next) => {
  ...
  const appStackLine = err.stack
    .split("\n")
    .find(
      (line) => !line.includes("node_modules") && line.includes(process.cwd())
    ); // Looks for your app's root directory

  const fileName = appStackLine
    ? appStackLine.match(/\((.*):\d+:\d+\)/)?.[1]
    : "Unknown File";
  const lineNumber = appStackLine
    ? appStackLine.match(/:(\d+):\d+\)/)?.[1]
    : "Unknown Line";

  const errorLogData = {
    service_name: process.env.ServiceName || "Service-Desk",
